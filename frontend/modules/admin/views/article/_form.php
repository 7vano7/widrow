...
<div class="coach-form">
            ...
            <div class="tab-content">
                <div id="panel1" class="tab-pane fade in active">
                    <div class="col-sm-12 down">
                        ...
                        <?= $form->field($model, 'category')->dropDownList((new Menu())->List(), ['prompt' =>
                            Yii::t
                        ('article', 'Select category')]) ?>
                        <?= $form->field($model, 'status')->dropDownList($model->getStatuses(), ['prompt' => Yii::t
                        ('article', 'Select status')]) ?>
                        <?= $form->field($model, 'top')->dropDownList($model->getTop(), ['prompt' => Yii::t
                        ('article', 'Select top')]) ?>
                        <?= $form->field($model, 'url')->textInput(['placeholder'=>Yii::t('article', 'Input url')]);?>
                        <?php if ($model->gif): ?>
                            <img src="<?php echo $model->gif ?>" style="width: 100px; margin: 10px">
                        <?php endif ?>
                        <?= $form->field($model, 'gif')->fileInput(['maxlength' => true]) ?>
    ...
    <?php else: ?>
        ...
        <div class="article-languages">
            <?= $form->field($model, 'category')->dropDownList($model->getCategory(Yii::$app->language), ['prompt' =>
                Yii::t
            ('article', 'Select category')]) ?>
            <?= $form->field($model, 'status')->dropDownList($model->getStatuses(), ['prompt' =>
                Yii::t('article', 'Select status')]) ?>
            <?= $form->field($model, 'top')->dropDownList($model->getTop(), ['prompt' =>
                Yii::t('article', 'Select top')]) ?>
            <?= $form->field($model, 'image')->fileInput(['maxlength' => true]) ?>
            <?php $i = 1; ?>
...
$script = "
    $(document).on('click', '.add-lang', function (event) {
        ...
        $('.add-lang').remove();
        $.ajax({
            url: '" . Url::toRoute('/admin/article/language') . "',
            type:'post',
...
$slug = " $('#article-url').slugify('input[name=\"ArticleTranslation[0][title]\"]');";
