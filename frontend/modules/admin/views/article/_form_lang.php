...
<div class="form-group field-articletranslation-content <?= $model->getErrors('content') ? 'has-error' : '' ?>">
    <label class="control-label" for="articletranslation-content"><?php echo Yii::t('article', 'Content') ?></label>
<!--    --><?php //echo Html::activeTextarea($model, 'content', ['aria-required' => true, 'placeholder' => Yii::t('article', 'Input content'), 'class' => 'form-control', 'name' => 'ArticleTranslation[' . $index . '][content]']) ?>
    <?php echo CKEditor::widget([
        ...
        'name' => 'ArticleTranslation[' . $index . '][content]',
        'id' => 'cke' . $index,
        'value' => $model->content,
    ]) ?>
