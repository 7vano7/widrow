...
use frontend\modules\rbac\models\BizRuleModel;
use frontend\modules\rbac\models\search\BizRuleSearch;
...
class RuleController extends Controller
{
    ...
    {
       if (!Yii::$app->user->can('access.rule.listview')) {
    ...
    {
       if (!Yii::$app->user->can('access.rule.view')) {
           throw new ForbiddenHttpException(Yii::t("access rules", "You haven't access to view this page"));
       }
    ...
    {
       if (!Yii::$app->user->can('access.rule.create')) {
           throw new ForbiddenHttpException(Yii::t("access rules", "You haven't access to view this page"));
       }
        $model = new BizRuleModel();

        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            Yii::$app->session->setFlash('success', Yii::t('rbac', 'Rule has been saved.'));
    ...
    {
       if (!Yii::$app->user->can('access.rule.update')) {
           throw new ForbiddenHttpException(Yii::t("access rules", "You haven't access to view this page"));
       }
        $model = $this->findModel($id);
    ...
    public function actionDelete(string $id)
    {
       if (!Yii::$app->user->can('access.rule.delete')) {
        ...
        Yii::$app->session->setFlash('success', Yii::t('rbac', 'Rule has been deleted.'));
    ...
    protected function findModel(string $id)
    {
        ...
        throw new ForbiddenHttpException(Yii::t("rbac", "he requested page does not exist."));
