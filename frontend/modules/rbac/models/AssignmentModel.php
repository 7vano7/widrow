...
class AssignmentModel extends BaseObject
{
     ...
     * @return bool
     */
    public function revoke(array $items, $id): bool
    {
        ...
        foreach ($items as $name) {
            ...
            $user = new User;
            if(count($roles) > 1 || $name != $user::ROLE_USER)
            {
                $item = $this->manager->getRole($name);
                $item = $item ?: $this->manager->getPermission($name);
                $this->manager->revoke($item, $this->userId);
                if(count($manager->getAssignments($id)) == 0)
                {
                    $manager->assign($manager->getRole($user::ROLE_USER), $id);
                }
            }
    ...
    public function attributeLabels()
    {
        return [
            'ID'=>Yii::t('admin', 'ID'),
            'username'=>Yii::t('access', 'Username')
        ];
    }
}
