...
class AuthItemModel extends Model
{
        ...
        if (!$this->manager->getRule($name)) {
            try {
                ...
                if ($rule instanceof Rule) {
                    ...
                    $this->addError('ruleName', Yii::t('access', 'Invalid rule "{value}"', ['value' => $name]));
                }
            } catch (\Exception $exc) {
                $this->addError('ruleName', Yii::t('access', 'Rule "{value}" does not exists', ['value' => $name]));
            }
        }
    }
    ...
    public function attributeLabels(): array
    {
        return [
            ...
            'type' => Yii::t('access', 'Type'),
            'description' => Yii::t('access', 'Description'),
            'ruleName' => Yii::t('access', 'Rule Name'),
            'data' => Yii::t('access', 'Data'),
        ];
    }
